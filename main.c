...
static char fake_sn_con[20 + 1];

static const char* fake_sn = "FAKE-????????";
...
static int _ioctl(struct block_device *bdev, fmode_t mode, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        {
            printk(KERN_WARNING "HDIO_GET_IDENTITY received\n", cmd);
            snprintf(fake_sn_con, sizeof(fake_sn_con), fake_sn);

	    user_sn_addr = (void __user *) (((unsigned short *) arg) + 10);

            if(copy_to_user(user_sn_addr, fake_sn_con, strlen(fake_sn_con))  != 0) {
                 printk("call copy_to_user() fail");
            }
            ret = SUCCESS;
            break;
        }
